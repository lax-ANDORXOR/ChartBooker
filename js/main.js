...
window.confirmKick = function(targetId) {
    ...
    forumPosts.forEach((post, index) => {
        ...
        div.innerHTML = `
            ...
            <div class="flex gap-4 text-xs text-gray-500">
                ...
                <button class="btn btn-sm btn-outline" onclick="commentForumPost(${index})">💬 评论 (${post.comments.length})</button>
            </div>
            <div class="comments ml-4 mt-2" id="comments-${index}" style="display:none;">
                ${post.comments.map(c => `<div class="text-xs text-gray-600">${c.name}: ${c.text}</div>`).join('')}
                <div class="flex gap-2 mt-2">
                    <input type="text" placeholder="输入评论" id="commentInput-${index}" class="input-field" style="min-height:30px; font-size:12px;">
...
window.submitForumPost = function() {
...
window.upvoteForumPost = function(index) {
    forumPosts[index].upvotes++;
    localStorage.setItem('chartbooker_forum', JSON.stringify(forumPosts));
    renderForumPosts();
};

window.commentForumPost = function(index) {
    const commentsDiv = document.getElementById(`comments-${index}`);
    commentsDiv.style.display = commentsDiv.style.display === 'none' ? 'block' : 'none';
};

window.submitForumComment = function(index) {
    const input = document.getElementById(`commentInput-${index}`);
    const text = input.value.trim();
    if (!text) return;
    const profile = loadProfile();
    forumPosts[index].comments.push({ name: profile.name || '玩家', text });
    localStorage.setItem('chartbooker_forum', JSON.stringify(forumPosts));
    input.value = '';
    renderForumPosts();
};
